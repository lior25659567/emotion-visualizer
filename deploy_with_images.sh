...
# 🚀 Deploy with All Images to Netlify
# This script packages the static files with all PNG images

echo "🚀 Deploying with all images to Netlify..."
...
DEPLOY_DIR="complete_netlify_deploy_$(date +%Y%m%d_%H%M%S)"
echo "📦 Creating complete deployment package: $DEPLOY_DIR"
mkdir -p "$DEPLOY_DIR"
...
# Create additional PNG directory structure that the website expects
echo ""
echo "🖼️  Creating PNG directory structure..."
mkdir -p "$DEPLOY_DIR/assets/conversion images"
mkdir -p "$DEPLOY_DIR/assets/logo"

# Copy all PNG images to the expected location
if [ -d "assets/conversion images" ]; then
    cp -r "assets/conversion images"/* "$DEPLOY_DIR/assets/conversion images/"
    echo "   ✅ Copied all PNG images"
fi

if [ -d "assets/logo" ]; then
    cp -r "assets/logo"/* "$DEPLOY_DIR/assets/logo/"
    echo "   ✅ Copied logo images"
fi

# Also copy any PNG files from the root directory
if [ -f "convo74.png" ]; then
    cp "convo74.png" "$DEPLOY_DIR/"
    echo "   ✅ Copied convo74.png"
fi
...
echo ""
echo -e "${GREEN}🎉 Complete deployment package created successfully!${NC}"
echo ""
...
echo ""
echo "🖼️  PNG images included:"
PNG_COUNT=$(find "$DEPLOY_DIR" -name "*.png" | wc -l)
echo "   ✅ $PNG_COUNT PNG images"
...
echo ""
echo "💡 This is a COMPLETE static deployment:"
echo "   • All frontend files included"
echo "   • ALL PNG images included"
echo "   • No Python backend needed"
echo "   • index.html will work perfectly"
echo "   • All images should display correctly"
echo ""
echo "✨ Your emotion visualizer will be live on Netlify with all images!" 